...
- (id)initWithKeyPath:(NSString*)theKeyPath {
    ...
    keyPath = theKeyPath ;
    timing = PNPropertyAnimationTimingEaseInEaseOut;
    duration = 0.5;
    startDelay = 0.0;
    return self;
}
...
- (void)update:(id)sender {
    ...
    for ( PNTPPropertyAnimation *animation in [animations copy] ) {
        ...
        switch ( animation.timing ) {
            case PNPropertyAnimationTimingEaseIn:
                position = funcQuad(time, 0.0, 1.0);
                break;
            case PNPropertyAnimationTimingEaseOut:
                position = funcQuadOut(time, 0.0, 1.0);
                break;
            case PNPropertyAnimationTimingEaseInEaseOut:
                position = funcQuadInOut(time, 0.0, 1.0);
                break;                
            case PNPropertyAnimationTimingLinear:
            default:
                break;
        }
